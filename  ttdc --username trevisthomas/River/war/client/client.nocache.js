...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "client" because\n';
  ...
  $wnd.__gwt_activeModules['client'] = {
    'moduleName' : 'client',
    'bindings' : function() {
      return {};
    }
 ...
 * "/client.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['client'].superdevmode = true;

  var expectedSuffix = '/client.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + client + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:client';
  var devModeSessionKey = '__gwtDevModeSession:client';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/client/client.nocache.js';
  ...
  $wnd.__gwt_activeModules['client'].canRedirect = true;
  $wnd.__gwt_activeModules['client'].superdevmode = true;
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/client?user.agent=' + ua + '&_callback=' + callback;
