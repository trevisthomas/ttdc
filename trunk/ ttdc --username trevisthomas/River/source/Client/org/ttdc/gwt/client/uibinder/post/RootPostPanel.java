 ...
 *
 */
public class RootPostPanel extends Composite implements PostPresenterCommon, PostEventListener, MessageEventListener, PersonEventListener{
    interface MyUiBinder extends UiBinder<Widget, RootPostPanel> {}
    ...
    @UiField DivElement bodyElement;
//    @UiField(provided = true) Widget avatarElement;
    ...
    @UiField Label replyCountLabelElement;
    @UiField Label conversationCountLabelElement;
...
//    @UiField TableCellElement avatarCell;
    ...
    @Inject
    public RootPostPanel(Injector injector) {
...
//		}
		...
		conversationCountElement.setText(""+post.getReplyCount());
			...
			if(post.getReplyCount() == 0){
				conversationCountElement.setTitle( "No conversations on this topic");
				conversationCountLabelElement.setText("Conversations");
			}
			else if(post.getReplyCount() == 1){
				conversationCountElement.setTitle( "One conversation in this topic");
				conversationCountLabelElement.setText("Conversation");
			}
			else{
				conversationCountElement.setTitle( post.getMass() + " conversations on this topic.");
				conversationCountLabelElement.setText("Conversations");
			}
			
			if(post.getMass() == 0){
				replyCountElement.setTitle( "No replies.");
				replyCountLabelElement.setText("Posts");
			}
			else if(post.getMass() == 1){
				replyCountElement.setTitle( "One reply.");
				replyCountLabelElement.setText("Post");
			}
			else{
...
//			}
	...
	private void refreshPost(GPost post) {
		...
		PostPanelHelper.highlightReadState(post,postTable,null);
