...
public class SplitHomePanel extends BasePageComposite implements PersonEventListener, PostEventListener, TagEventListener, MessageEventListener {
	interface MyUiBinder extends UiBinder<Widget, SplitHomePanel> {}
	...
	@UiField VerticalPanel sidebarElement;
	...
	private void buildTopicTab(){
		if(PresenterHelpers.isWidgetEmpty(topicPanel)){
			topicPresenter = injector.getFlatPresenter();
			topicPresenter.init(FlatPresenter.FlatContentType.LATEST_TOPICS);
			topicPanel.clear();
			topicPanel.add(topicPresenter.getWidget());
		}
	}
